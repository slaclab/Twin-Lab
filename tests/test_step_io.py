from __future__ import annotations

from pathlib import Path

import pytest

ocp = pytest.importorskip("OCP")
pytest.importorskip("trimesh")

from OCP.BRepBuilderAPI import BRepBuilderAPI_Transform
from OCP.BRepPrimAPI import BRepPrimAPI_MakeBox
from OCP.IFSelect import IFSelect_RetDone
from OCP.STEPControl import STEPControl_AsIs, STEPControl_Writer
from OCP.gp import gp_Trsf, gp_Vec

from slac_robotics.step_io import detect_step_interferences, load_step_mesh


def _write_box_step(path: Path, *, size_xyz: tuple[float, float, float], offset_xyz: tuple[float, float, float]) -> None:
    shape = BRepPrimAPI_MakeBox(*size_xyz).Shape()
    if offset_xyz != (0.0, 0.0, 0.0):
        trsf = gp_Trsf()
        trsf.SetTranslation(gp_Vec(*offset_xyz))
        shape = BRepBuilderAPI_Transform(shape, trsf, True).Shape()

    writer = STEPControl_Writer()
    writer.Transfer(shape, STEPControl_AsIs)
    status = writer.Write(str(path))
    if status != IFSelect_RetDone:
        raise RuntimeError(f"Failed to write STEP file: {path}")


def test_load_step_mesh_and_detect_collision(tmp_path: Path) -> None:
    left = tmp_path / "left.step"
    right = tmp_path / "right.step"

    _write_box_step(left, size_xyz=(0.20, 0.20, 0.20), offset_xyz=(0.0, 0.0, 0.0))
    _write_box_step(right, size_xyz=(0.20, 0.20, 0.20), offset_xyz=(0.12, 0.0, 0.0))

    left_mesh = load_step_mesh(left)
    assert len(left_mesh.vertices) > 0
    assert len(left_mesh.faces) > 0

    collisions = detect_step_interferences([("left", left), ("right", right)])
    pairs = {frozenset((c.a, c.b)) for c in collisions}
    assert frozenset(("left", "right")) in pairs


def test_detect_step_no_collision(tmp_path: Path) -> None:
    left = tmp_path / "left.step"
    right = tmp_path / "right.step"

    _write_box_step(left, size_xyz=(0.20, 0.20, 0.20), offset_xyz=(0.0, 0.0, 0.0))
    _write_box_step(right, size_xyz=(0.20, 0.20, 0.20), offset_xyz=(1.00, 0.0, 0.0))

    collisions = detect_step_interferences([("left", left), ("right", right)])
    assert collisions == []
