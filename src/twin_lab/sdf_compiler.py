...
def compile_sdf_package(
    ...
    *,
    ...
    if package_dir.exists():
        if not (package_dir / PACKAGE_MARKER_NAME).exists():
...
def package_is_current(
    ...
    *,
    ...
    A package written before build stamps has no readable key, so it recompiles once.
    """

    stamp = Path(package_dir) / PACKAGE_MARKER_NAME
    if not stamp.exists():
        return False
    try:
    ...
    scene_file = Path(scene_path).resolve()
    scene = yaml.safe_load(scene_file.read_text(encoding="utf-8"))
    links, _ = _build_tree(scene, scene_file)
    return recorded == _package_build_key(
        scene_file,
        scene,
        links,
        model_name=model_name,
