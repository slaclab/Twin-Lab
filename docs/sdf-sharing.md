...
CAD pose. For `A048` and `A053`, the logical display angle is the SDF coordinate
plus 180 degrees. These references follow the STEP revision, so read the offsets
from the generated `joint_metadata.csv` rather than from this page.
